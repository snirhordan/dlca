...
from addon.weighted_sum import WeightedSum
...
def report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
    ...
    number_traj = len(traj_clean_crit_list[1])
    csv_path = os.path.join(output_dir, 'results_' + crit_str + '.csv')
    img_path = os.path.join(output_dir, 'results_' + crit_str + '_image.png')
    ...
    for traj_idx, traj_name in enumerate(traj_name_list):
        ...
        #if crit_str == "rms" and experiment_name == "test":
        #    idx = [ idx for j in range(number_traj) for idx in range(traj_len)]
        #    print("index list is as follows: \n")
        #    print(idx)
        #    ratio_list = torch.tensor(traj_ratio_crit_list).view(-1)
        #    print("length of ratio list is as follows:\n")
        #    print("{}".format(len(ratio_list)))
        #    data = { "ratio" : ratio_list, "frame_idx" : idx }
        #    data = pd.DataFrame(data = data)
        #    sns_plot = sns.lineplot(x="indx", y="values", data=data)
        #    sns_plot.figure.savefig(img_path)
        ####################################################################################
...
def adv_oos_test_perturb_trajectories(args, train_traj_names, eval_traj_names, test_traj_names,
    ...
    args.best_patches.append(best_pert)#save best pert
    ...
    args.best_loss.append(eval_best_loss_list)
...
def run_attacks_out_of_sample_test(args):
    ...
    for test_fold_idx in list(range(1)):
        output_dir, adv_best_pert_dir, \
        train_output_dir, train_flowdir, train_pose_dir, train_img_dir, train_adv_img_dir, train_adv_pert_dir, \
        eval_output_dir, eval_flowdir, eval_pose_dir, eval_img_dir, eval_adv_img_dir, eval_adv_pert_dir, \
        test_output_dir, test_flowdir, test_pose_dir, test_img_dir, test_adv_img_dir, test_adv_pert_dir = \
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size,
                                         shuffle=False, num_workers=args.worker_num) #stochastic GRADIENT DESCENT with batch_size=1
    ...
    #from here args.best_patches and args.best_loss are populated
    #softmax each best perturbation and aggragate into one
    vector = [torch.sum(torch.tensor(item[0])) for item in args.best_loss]#best lst frame loss
    print("last frame loss function")
    print(vector[0])
    vector = torch.nn.functional.softmax(torch.tensor(vector)) #softmax sum to 1
    print("softmax vector is")
    print(vector)
    inputs = args.best_patches#list of images of best perturbation , why [0]
    print("shape of pathch[0] is ")
    print( "({})".format( tuple((inputs[0][0]).size()) ) ) 
    interpolated = torch.zeros_like(inputs[0].clone().detach())
    for idx, item in enumerate(inputs):
        interpolated += torch.mul( item,vector[idx] )
        interpolated.clamp_(0, 1)
    save_image(interpolated[0], adv_best_pert_dir + '/' + 'adv_interpolation.png') 
    #save all adversarial perturbations in one folder
    for idx in range(1):
        save_image(args.best_patches[idx][0], adv_best_pert_dir + '/' + 'adv_best_pert_{}.png'.format(idx+1))
        torch.save(args.best_patches[idx][0], adv_best_pert_dir + '/' + 'adv_best_pert_tensor{}.pt'.format(idx+1))
